...
if git rev-parse --git-dir >/dev/null 2>&1 && [[ -n "$CLAUDE_TOOL_FILE_PATH" ]]; then
  ...
  if [[ $CHANGED_LINES -gt 0 ]]; then
    ...
    if [[ $CHANGED_LINES -lt 10 ]]; then SIZE="minor"; elif [[ $CHANGED_LINES -lt 50 ]]; then SIZE="moderate"; else SIZE="major"; fi
    MSG="Update $FILENAME: $SIZE changes ($CHANGED_LINES lines)"
    git commit -m "$MSG" "$CLAUDE_TOOL_FILE_PATH" 2>/dev/null || true
  fi
fi
