You have access to the `gh` terminal command. I already authenticated it for you. Please review it to use the PR that I asked you to review. You're already in the `ava-module-monorepo` repo.

**Available Tools:**
- `run_terminal_cmd` - Execute terminal commands (including `gh` commands)
- `read_file` - Read file contents from the codebase
- `grep` - Search for patterns in files with regex support
- `codebase_search` - Semantic search to understand code by meaning
- `glob_file_search` - Find files by name patterns
- Other standard tools for analysis and review
...
2. Examine the original files in the main branch to understand the context using the `read_file` tool

3. For specific sections of a file or to search for patterns, use the `grep` tool with regex patterns and optional file type filters
...
1. Before making a decision, ask the user directly if you should approve the PR, providing your assessment and justification. Present a clear summary of:
   ...
   - Ask if they'd like you to proceed with this recommendation
...
1. After the user decides on approval/rejection, ask if they would like a comment drafted that they can copy and paste
...
Use the `read_file` tool to examine the original files to understand what's being changed:
- Read `src/shared/api.ts`
- Read `webview-ui/src/components/settings/ThinkingBudgetSlider.tsx`

Use the `grep` tool to check how thinking mode is currently handled in API providers:
- Search for "reasoningOn" pattern in `src/api/providers` directory with `*.ts` file type filter
...
Ask the user directly with your assessment:
...
Ask the user: "Would you like me to draft a comment for this PR that you can copy and paste?"
