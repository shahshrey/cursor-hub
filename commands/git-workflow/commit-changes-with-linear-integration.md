...
<task_objective>
Create conventional commits in a monorepo while integrating required Linear ticket references. This command helps enforce feature-branch usage, include ticket IDs, exclude sensitive files, update Linear status, and format commits as `<type>(TIC-XXX): <description>`.
</task_objective>
...
2. **If on `main`**:
   ...
   a. Prompt the user to provide the Linear ticket number in format `TIC-XXX`
   ...
   d. Create and switch to a new feature branch using Linear ticket format:
      ```bash
      git checkout -b TIC-123-add-reset-password
      ```
      Format: `TIC-XXX-description`
...
- **Linear ticket reference** (TIC-XXX format)
...
2. Create commit with Linear ticket in format: `<type>(TIC-XXX): <description>`
   ```bash
   git commit -m "feat(TIC-123): add password reset endpoint" -m "Adds backend endpoint and tests" -m "Related to TIC-123"
   ```
...
git status
git commit -m "feat(TIC-123): add password reset support" -m "Includes endpoint and test coverage"
git status

# Commit 2: Documentation
git add README.md
git status
git commit -m "docs(TIC-123): document password reset flow" -m "Updates usage instructions"
git status
```
...
- **scope**: Optional - Linear ticket (TIC-XXX) or component name
...
git commit -m "feat(TIC-123): add JWT token validation middleware"

# Bug fix
git commit -m "fix(TIC-456): resolve avatar upload sizing issue"

# Breaking change
git commit -m "feat(TIC-789)!: send email when product is shipped"

# With body
git commit -m "refactor(TIC-234): replace admin_client with slack_admin_bot" -m "Updates architecture to use new bot pattern"

# Documentation
git commit -m "docs(TIC-567): update installation instructions"

# Tests
git commit -m "test(TIC-890): add comprehensive JWT validation tests"

# Dependencies
git commit -m "chore(TIC-111): upgrade fastapi to version 0.104.1"
```
...
✅ **Always include Linear ticket** - Format: TIC-XXX
